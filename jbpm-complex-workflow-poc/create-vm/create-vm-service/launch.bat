...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "create-vm-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name create-vm-service apps/create-vm-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app create-vm-service:1.0-SNAPSHOT
		call oc expose svc/create-vm-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd create-vm-service
		cd target
