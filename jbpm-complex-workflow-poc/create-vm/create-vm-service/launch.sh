...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "create-vm-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name create-vm-service apps/create-vm-service:2.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app create-vm-service:2.0-SNAPSHOT
    oc expose svc/create-vm-service
else

	echo "Launching the application locally..."
	pattern="create-vm-service"
	files=( $pattern )
	cd ${files[0]}
